...
import fetchLookupData from '@salesforce/apex/ND_CustomLookupLwcController.fetchLookupData';
import fetchDefaultRecord from '@salesforce/apex/ND_CustomLookupLwcController.fetchDefaultRecord';
import Id from '@salesforce/user/Id';

const DELAY = 300; // dealy apex callout timing in miliseconds  

export default class CustomUserLookup extends LightningElement {
    // public properties with initial default values 
    @api label = 'Fundraiser';
    @api placeholder = 'Search People...'; 
   ...
   // initial function to populate default selected lookup record if defaultRecordId provided  
    connectedCallback(){
            ...
            fetchDefaultRecord({ recordId: this.defaultRecordId!=null?this.defaultRecordId:Id , 'sObjectApiName' : this.sObjectApiName })
            .then((result) => {
                if(result != null){
...
this.dispatchEvent(oEvent);
    ...
    *   that we need to validate are valid or not.S
    */
    @api
    isInputValid() {
